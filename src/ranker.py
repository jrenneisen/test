...
def score_jobs(
    ...
    for job_id, emb_score in candidate_scores:
        ...
        final = (
            weights.get("embedding_similarity", 0.35) * float(emb_score) +
            weights.get("skill_match",          0.25) * skill_match +
            weights.get("title_match",          0.15) * title_match +
            weights.get("location_fit",         0.10) * loc_fit +
        ...
        why = _build_explanation(
            row, emb_score, matched, missing, title_match, loc_fit, sal_fit, sal_min_pref
        )
