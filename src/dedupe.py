...
def deduplicate_exact(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    """
    Remove exact duplicates based on (title, company, location).
    Returns cleaned DataFrame and a stats dict.
    """
    before = len(df)
    key = df["title"].str.lower().str.strip() + "|" + \
    ...
    logger.info(f"Exact dedup: removed {removed:,} rows ({stats['rate']:.1%} of total)")
    return df, stats
